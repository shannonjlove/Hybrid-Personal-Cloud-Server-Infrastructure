...
| **Kaleidoscope** (licensed 2021, Letter Opener GmbH) | Verified directly against kaleidoscope.app: the entire site mentions only "macOS" — no Windows, no Linux, anywhere. The download page detects a Mac user-agent and serves "Kaleidoscope 4" specifically. Same category as NeoFinder: no cross-platform build has ever existed. | **Not containerizable, same reason as NeoFinder.** Redundant with Beyond Compare 4 Pro for the diff/merge use case anyway — no Linux replacement needed. Stays a Mac-side tool for local use on your actual Mac, alongside the shelved DeltaWalker v4 license. |
...
- Add `02-CONTAINERS/webtopsjl/` (`linuxserver/webtop` Quadlet, `sjl.exposure=private`)
  ...
  repo's services list regardless of the answer, since it
